...
def build_post_from_wiki(
    *,
    ...
    if kind == "place":
        ...
        theme = rng.choices(
            ["避坑", "路线", "求助", "接送站", "寄宿家庭", "投诉"],
            weights=[18, 18, 20, 16, 10, 18],
            k=1,
        )[0]
        if theme == "路线":
            ...
            post_title = f"到{region or '潮汕'}下站/转车：有没有顺路接送站/指路？"
            body = (
                f"我准备到 **{display}** 一带落脚，行程比较赶，对下站/转车这段不太熟。\n\n"
                "## 我需要的帮助\n"
                "- 下站后怎么走/坐什么车更稳？（尤其是晚点/下雨这种情况）\n"
                "- 如果你顺路方便，能不能帮忙 **接送站/指路**？（完全自愿，不强求）\n\n"
                "## 安全与规则（重要）\n"
                "- 不要在帖子里公开手机号/微信/银行卡等信息\n"
                "- 线下对接前先核验身份，明确费用/时间/上车点/车牌\n"
                "- 遇到可疑信息请直接举报/拉黑\n\n"
                "谢谢胶己人，先抱歉打扰了。\n"
            )
            post_tags = normalize_tags(tags + ["接送站", "求助"])
        elif theme == "寄宿家庭":
            post_title = f"{region or '潮汕'}临时借宿/落脚：有没有寄宿家庭/好心人？"
            body = (
                f"我准备去 **{display}** 一带，行程有点变化，可能需要临时借宿/临时落脚（1 晚或半天）。\n\n"
                "## 我会说明清楚\n"
                "- 时间：今晚/明晚（可具体到几点）\n"
                "- 人数：1~2（可补充）\n"
                "- 预算/心意：尽量按市场价，不白住\n\n"
            ...
            post_tags = normalize_tags(tags + ["寄宿家庭", "求助"])
        elif theme == "投诉":
            post_title = f"{display} 附近遇到不愉快经历/疑似被坑，求大家帮忙避雷"
            body = (
                f"我在 **{display}** 附近遇到一次不太愉快的体验，想发出来提醒大家，也想请本地胶己人帮我判断：这算不算正常？\n\n"
                "## 我遇到的情况\n"
                "- 事情经过：我会尽量客观描述\n"
                "- 大概价格/时间：可补充\n\n"
                "## 我想确认\n"
                ...
                "- 同类情况常见吗？怎么避免？\n\n"
                "## 说明\n"
                "- 为了避免误伤，这里不公开个人隐私与可识别信息\n"
                "- 欢迎理性讨论，不造谣不人身攻击\n\n"
                "给你添麻烦了，先抱歉。\n"
            )
            post_tags = normalize_tags(tags + ["投诉", "避坑"])
        else:
